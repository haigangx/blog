...
`tcpdump` 可以让超级用户监视网络中的实时流量，实时生成文本信息。它可以用来显示所有类型的 TCP 数据包流量（如：TCP 报文、UDP 数据报以及 ICMP 报文）。对于每个网络报文，`tcpdump` 都会显示出像时间戳、源 IP 地址、目的 IP 地址以及更多协议特有的细节信息，而且可以根据协议类型、源和目的 IP 地址、端口号以及其他一些标准来选择需要监视的数据包
