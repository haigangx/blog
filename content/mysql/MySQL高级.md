...
- [1.MySQL环境](#1mysql环境)
    ...
    - [1.3.修改字符集](#13修改字符集)
    - [1.4.配置文件](#14配置文件)
- [2.MySQL逻辑架构](#2mysql逻辑架构)
- [3.存储引擎](#3存储引擎)
- [4.SQL性能下降的原因](#4sql性能下降的原因)
- [5.SQL执行顺序](#5sql执行顺序)
- [6.七种JOIN理论](#6七种join理论)
- [7.索引](#7索引)
    - [7.1.索引简介](#71索引简介)
    - [7.2.MySQL索引分类](#72mysql索引分类)
    - [7.3MySQL索引数据结构](#73mysql索引数据结构)
    - [7.4.哪些情况需要建索引](#74哪些情况需要建索引)
    - [7.5.那些情况不要建索引](#75那些情况不要建索引)
- [8.性能分析](#8性能分析)
    - [8.1 MySQL Query Optimizer](#81-mysql-query-optimizer)
    - [8.2 MySQL 常见瓶颈](#82-mysql-常见瓶颈)
    - [8.1.EXPLAIN 简介](#81explain-简介)
    - [8.2.EXPLAIN字段](#82explain字段)
- [9.索引分析](#9索引分析)
    - [9.1.单表索引分析](#91单表索引分析)
    - [9.2.两表索引分析](#92两表索引分析)
    - [9.3.三张表索引分析](#93三张表索引分析)
    - [9.4.结论](#94结论)
- [10.索引失效](#10索引失效)
    - [10.1.索引失效的情况](#101索引失效的情况)
    - [10.2.最佳左前缀法则](#102最佳左前缀法则)
    - [10.3.索引列上不计算](#103索引列上不计算)
    - [10.4.范围之后全失效](#104范围之后全失效)
    - [10.5.覆盖索引尽量用](#105覆盖索引尽量用)
    - [10.6.不等有时会失效](#106不等有时会失效)
    - [10.7.like百分加右边](#107like百分加右边)
    - [10.8.字符要加单引号](#108字符要加单引号)
    - [10.9.索引相关题目](#109索引相关题目)
    - [10.10.面试题分析](#1010面试题分析)
    - [10.11.总结](#1011总结)
- [11.分析慢SQL的步骤](#11分析慢sql的步骤)
- [12.查询优化](#12查询优化)
    - [12.1.小表驱动大表](#121小表驱动大表)
    - [12.2.ORDER BY优化](#122order-by优化)
    - [12.3.GORUP BY优化](#123gorup-by优化)
    - [12.4.总结](#124总结)
- [13.慢查询日志](#13慢查询日志)
    - [13.1.基本介绍](#131基本介绍)
    - [13.2.日志分析工具](#132日志分析工具)
- [14.批量插入数据脚本](#14批量插入数据脚本)
    - [14.1.环境准备](#141环境准备)
    - [14.2.创建函数](#142创建函数)
    - [14.3.创建存储过程](#143创建存储过程)
    - [14.4.调用存储过程](#144调用存储过程)
- [15.Show Profile](#15show-profile)
- [16.表锁(偏读)](#16表锁偏读)
    - [16.1.环境准备](#161环境准备)
    - [16.2.锁表的命令](#162锁表的命令)
    - [16.3.读锁案例](#163读锁案例)
...
- [17.行锁(偏写)](#17行锁偏写)
    - [17.1.环境准备](#171环境准备)
    - [17.2.行锁案例](#172行锁案例)
    - [17.3.索引失效行锁变表锁](#173索引失效行锁变表锁)
    - [17.4.间隙锁的危害](#174间隙锁的危害)
    - [17.5.如何锁定一行](#175如何锁定一行)
    - [17.6.案例结论](#176案例结论)
    - [17.7.行锁分析](#177行锁分析)
- [18.主从复制](#18主从复制)
    - [18.1.复制基本原理](#181复制基本原理)
    - [18.2.复制基本原则](#182复制基本原则)
    - [18.3.一主一从配置](#183一主一从配置)
...
MySQL 官方对索引的定义为：**索引（INDEX）是帮助 MySQL 高效获取数据的数据结果。**
...
我们平时所说的索引，如果没有特别指明，都是指 B 树（多路搜索树，并不一定是二叉的）结构组织的索引。其中聚集索引，次要索引，覆盖索引，复合索引，前缀索引，唯一索引默认都是使用 B+ 树索引，统称索引。当然，除了 B+ 树这种数据结构的索引之外，还有哈希索引（Hash Index）等。
...
- 查找：类似大学图书馆的书目索引，提高数据检索的效率，降低数据库的 IO 成本。
- 排序：通过索引対数据进行排序，降低数据排序的成本，降低了 CPU 的消耗。
...
- 经常增删改的表
  ...
  提高了查询速度，同时却会降低更新表的速度，如对表进行 `INSERT`、`UPDATE`、`DELETE`，因为更新表时，MySQL 不仅要保存数据，还要保存索引文件

- 频繁更新的字段不适合创建索引
- Where条件里用不到的字段不创建索引
- 数据重复且分布平均的表字段不适合创建索引，应该只为最经常查询和最经常排序的数据列建立索引，注意，如果某个数据列包含许多重复的内容，为它建立索引就没有太大的实际效果
...
MySQL 中有专门负责优化 `SELECT` 语句的优化器模块，主要功能：通过计算分析系统中收集到的统计信息，为客户端请求的 Query 提供他认为最优的执行计划（它认为最优的数据检索方式，但不见得是 DBA 认为最优的，这部分最耗费时间）

当客户端向 MySQL 请求一条 Query，命令解析器模块完成请求分类，区别出是 `SELECT` 并转发给 MySQL Query Optimizer 时，MySQL Query Optimizer 首先会对整条 Query 进行优化，处理掉一些常量表达式的预算，直接换算成常量值。并对 Query 中的查询条件进行简化和转换，如去掉一些无用或显而易见的条件、结构调整等。然后分析 Query 中的 Hint 信息（如果有），看显示 Hint 信息是否可以完全确定该 Query 的执行计划，如果没有 Hint 或 Hint 信息还不足以完全执行计划，则会读取所涉及对象的统计信息，根据 Query 进行写相应的计算分析，然后再得出最后的执行计划
...
EXPLAIN：SQL 的执行计划，使用 EXPLAIN 关键字可以模拟优化器执行 SQL 查询语句，从而知道 MySQL 是如何处理SQL语句的
...
- `UNION RESULT`：从`UNION`表获取结果的`SELECT`。
  ...
  |  1 | PRIMARY      | b          | NULL       | eq_ref | PRIMARY       | PRIMARY | 4       | db02.a.deptId |    1 |   100.00 | NULL                                       |
  |  2 | UNION        | b          | NULL       | ALL    | NULL          | NULL    | NULL    | NULL          |    5 |   100.00 | NULL                                       |
  |  2 | UNION        | a          | NULL       | ALL    | NULL          | NULL    | NULL    | NULL          |    8 |   100.00 | Using where; Using join buffer (hash join) |
...
- [视频教程](https://www.bilibili.com/video/BV1KW411u7vy?p=13)
- [视频教程网友笔记](https://blog.csdn.net/qq_36903261/article/details/108507212)
